...
func TestTranscodedFileName(t *testing.T) {
	...
	name := cs.transcodedFileName(local, "mp3", 0, -1, false)
	if name != "42.tc.mp3" {
		t.Errorf("transcodedFileName(local, 0) = %q, want %q", name, "42.tc.mp3")
	}

	// 本地歌曲，有 bitrate
	name = cs.transcodedFileName(local, "mp3", 128, -1, false)
	...
	name = cs.transcodedFileName(remote, "ogg", 0, -1, false)
	...
	// 插件来源歌曲，有 bitrate
	name = cs.transcodedFileName(remote, "mp3", 192, -1, false)
...
func TestFindTranscodedFile(t *testing.T) {
	...
	// 不存在时应 miss
	if _, ok := cs.FindTranscodedFile(song, "mp3", 0, -1, false); ok {
	...
	name := cs.transcodedFileName(song, "mp3", 0, -1, false)
	path := filepath.Join(dir, name)
	os.WriteFile(path, []byte("fake mp3"), 0644)

	// format-only 应命中
	found, ok := cs.FindTranscodedFile(song, "mp3", 0, -1, false)
	if !ok {
	...
	// 不同 bitrate 应 miss
	if _, ok := cs.FindTranscodedFile(song, "mp3", 320, -1, false); ok {
