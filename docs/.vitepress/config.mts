...
export default async () => {
  return withMermaid(defineConfig({
    ...
    // 分支临时件，不是产品文档：不建页、不进 sitemap、也刻意不做中英双语同步。
    // songloft-org/songloft#341 落地后连同这些 md 一起删除（这条 srcExclude 也删掉）。
    srcExclude: ['webf-migration-handoff.md', 'webf-recon-step456.md', 'webf-step4-design.md'],
    ...
    vite: {
      plugins: [
        AutoSidebar({
          ...
          ignoreList: ['webf-migration-handoff', 'webf-recon-step456', 'webf-step4-design'],
        }),
      ],
    },
  }))
