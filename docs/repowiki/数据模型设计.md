...
当前 schema 共 **8 张表**：`songs`、`playlists`、`playlist_songs`、`play_history`、`configs`、`auth_tokens`、`js_plugins`、`plugin_storage`，通过 30 个迁移文件从初始版本演进到当前状态。核心表的 `updated_at` 列均由 SQLite TRIGGER 自动维护。
