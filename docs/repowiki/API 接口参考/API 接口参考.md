...
> 详见 [播放历史 API](播放历史%20API.md)

| 前缀 | 端点数 | 说明 |
|------|--------|------|
| `/api/v1/play-history` | 3 | 按播放上下文（歌单 / 歌手 / 专辑 / 其余分面维度）记录与查询最近播放的歌曲 |

覆盖：查询某上下文的播放历史（含完整歌曲详情，上限 50 条不分页）、清空某上下文、删除单条记录。写入挂在 `POST /songs/{id}/played` 的 `context_type` + `context_key` 参数上，无独立写入端点。
