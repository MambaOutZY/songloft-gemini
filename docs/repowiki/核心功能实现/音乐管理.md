...
优先级：旁挂 `.lrc` 文件 > 内嵌歌词（tag）。`FindSidecarLyricFile` 按候选顺序匹配：
`<base>.lrc` / `.LRC` / `.Lrc`，然后 `<含扩展名>.lrc` / `.LRC` / `.Lrc`。
0 字节文件和同名目录视作未命中。`ReadSidecarLyric` 处理 UTF-16 BOM 解码和 GBK 修正（`tag.FixEncoding`）。
CUE 切分轨不读取旁挂歌词（整轨 lrc 时间轴套到单曲是错的）。

运行时 `GET /songs/{id}/lyric` 也会实时读盘检查旁挂，命中则优先于 DB 中的插件歌词，
并异步通过 `SyncSidecarLyric` 回写 DB（不回写音频标签）。`manual` 歌词不被旁挂覆盖。
...
ScanAndImportAsync
  ...
  +-- go doScanAndImport(ctx, reimport)
        ...
        |
        (8) runAutoFingerprint -- 自动计算缺失指纹（默认关闭，见 5.4）
```
...
- **采样上限 120 秒**（`fingerprintSampleSeconds`）：AcoustID 的事实标准，同时把长音频（有声书、整轨镜像）的开销压成常量。不设上限时 chromaprint 会解码整个文件
- **CUE 轨按区间采样**：CUE 轨的 `file_path` 指向整轨镜像，用 `-ss cue_start_seconds` 定位并把 `-t` 夹在轨内；否则同一镜像下所有 track 会拿到完全相同的指纹
- 超时：30 秒 context timeout，外加 `cmd.WaitDelay = 5s` 防 ffmpeg 派生子进程持有管道挂住 worker
- 输出：base64 编码的指纹字符串（从 stdout 读取，取第一行）
- 时长：从 stderr 解析 `Duration: HH:MM:SS.XX`（全片时长，不受 `-t` 影响）；CUE 轨改用轨长
...
| 方法 | 行为 |
|------|------|
| `ComputeMissing` | 为所有缺失指纹、且从未尝试过的本地歌曲计算指纹 |
| `RecomputeAll` | 清空所有已有指纹与「已尝试」标记后重新计算全部 |
| `Cancel` | 中断正在运行的任务并等待收尾，进度转为 `cancelled` |

若已有任务在运行，先 cancel 旧任务等待其完成再启动新任务。

`doCompute` 的并发度由 `fpWorkerCount()` = `clamp(GOMAXPROCS/4, 1, 4)` 决定（Go 的 GOMAXPROCS 感知 cgroup CPU 限额，Docker 限核能正确收敛），通过 channel 分发任务。进度通过 `FingerprintProgress`（status/computed/total/failed）实时可查。

**失败必须落库**：每个 worker 无论成败都写入 `songs.fingerprint_attempted_at`（unix 秒）——成功走 `UpdateFingerprint`，失败走 `MarkFingerprintAttempted`。`ListLocalWithoutFingerprint` 的条件是 `fingerprint = '' AND fingerprint_attempted_at = 0`，因此失败项只尝试一次。若失败不落库，AutoScanner 每轮都会把同一批注定失败的文件重新捞出来跑 ffmpeg（songloft-org/songloft#323）。任务被取消时中途的歌曲**不打**标记，下次续算。

### 5.4 触发时机与开关

扫描尾部的 `runAutoFingerprint`（`song_service.go`）**默认不执行**：config key `scan_auto_fingerprint` 默认 `false`，业务端点 `GET/PUT /api/v1/settings/scan-auto-fingerprint`。指纹只服务于重复检测和插件搜索，属按需功能，全库自动计算会在扫描进度已显示「完成」之后继续长时间占用 CPU。关闭时用户在「重复歌曲检测」页手动触发。

指纹任务用独立的 `context.Background()`，不复用扫描的 cancelCh（`scanProgressManager.Complete()` 已 `close(cancel); cancel = nil`，之后 `GetCancelChannel()` 返回 nil），故通过 `POST /api/v1/scan/fingerprints/cancel` 单独取消。

### 5.5 去重检测

指纹入库后可通过 `ListDuplicateGroups` 查询所有具有相同指纹的本地歌曲组。因为只采样前 120 秒，同指纹内还会按 `fingerprint_duration`（全片时长）以 30 秒容差**相邻**聚簇（时长为 0 表示未知，不作为切分依据），只有簇内 ≥2 首才算重复组——用于排除「统一片头的有声书/系列节目」这类前若干秒一致的碰撞。前端通过 `GET /songs/duplicates` 展示重复歌曲，支持通过 `POST /songs/batch-delete`（带 `delete_files=true`）删除重复文件。
...
| `/api/v1/scan/fingerprints/cancel` | POST | 中断指纹计算（返回 `cancelled`） |
| `/api/v1/settings/scan-auto-fingerprint` | GET/PUT | 「扫描后自动计算指纹」开关，默认 `false` |
