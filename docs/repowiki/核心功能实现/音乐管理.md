...
指纹入库后可通过 `ListDuplicateGroups` 查询所有具有相同指纹的本地歌曲组。因为只采样前 120 秒，同指纹内还会按 `fingerprint_duration`（全片时长）以 2 秒容差聚簇，只有簇内 ≥2 首才算重复组——用于排除「统一片头的有声书/系列节目」这类前若干秒一致的碰撞。前端通过 `GET /songs/duplicates` 展示重复歌曲，支持通过 `POST /songs/batch-delete`（带 `delete_files=true`）删除重复文件。
