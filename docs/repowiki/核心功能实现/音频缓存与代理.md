...
缓存总量超过 `max_size`（默认 1GB，0 表示不限制）时，按最后访问时间淘汰最旧的文件。淘汰算法分三阶段执行，通过分级键控制并发开销：

**章节来源**: `internal/services/cache_service.go`（EvictLRU、lruMaxHeap、lruHeapCap）

```mermaid
graph TD
    A["① 无锁遍历文件系统"] --> B["收集所有缓存文件信息<br/>（路径、大小、mtime）<br/>跳过 .tmp 临时文件"]
    B --> C{"totalSize <= maxSize?"}
    C -- "是" --> Z["未超限，直接返回"]
    C -- "否" --> D["② 持读锁查询 lruIndex<br/>获取内存中的访问时间"]
    D --> E["构建固定容量最大堆<br/>heapCap = max(indexSize/4, 128)<br/>堆顶为最新访问，保留最旧 N 个"]
    E --> F["候选文件按访问时间升序排序"]
    F --> G["③ 持写锁执行淘汰<br/>从最旧开始删除<br/>直到 totalSize <= maxSize"]
    G --> H{"仍超限?"}
...
1. **第一阶段（无锁遍历文件系统）**：`filepath.Walk` 收集所有缓存文件的路径、大小和 mtime。此阶段不持任何锁，避免慢速文件 I/O 阻塞缓存读写。临时文件（`.tmp` 后缀）在遍历时跳过。
2. **第二阶段（持读锁构建堆）**：持 `lruMu.RLock` 查询 `lruIndex`（内存中的访问时间戳），用 `container/heap` 维护固定容量最大堆（堆顶为最新访问的文件），遍历时若当前文件比堆顶更旧则替换堆顶，最终堆内即为最应淘汰的文件。堆容量 `heapCap = max(len(lruIndex)/4, 128)`，上限不超过索引总数，意味着单次淘汰最多评估 25% 的文件。
3. **第三阶段（持写锁执行删除）**：候选文件按访问时间升序排序后，持 `lruMu.Lock` 从最旧开始逐个删除，同步清理 `lruIndex` 和空目录（`cleanEmptyParentDirs`）。

若单轮淘汰后仍超限（候选不够），日志打 Warn，下次触发会继续清理。
