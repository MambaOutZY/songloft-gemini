...
| `FingerprintService` | 可选注入 | 扫描完成后自动计算指纹（默认关闭） |
...
| `FindLyricFile` / `ReadLyricFile` | 委托到 `FindSidecarLyricFile`/`ReadSidecarLyric`，支持大小写、双扩展名变体，UTF-16 BOM + GBK 解码 |
...
| `CleanCache` | 清理全部缓存 |
| `EvictLRU` | LRU 淘汰（三阶段：① 无锁遍历文件系统收集文件信息，② 持读锁查 lruIndex 构建最大堆，③ 持写锁执行淘汰删除） |
| `Shutdown` | 关闭 shutdownCtx，通知后台 ffmpeg 转码等 goroutine 尽快退出，由 app.Close 在应用关闭时调用 |
| `GetCacheConfig` / `UpdateCacheConfig` | 配置管理（`max_size`、`cache_dir`、`transcode_format`、`transcode_quality`） |
...
使用 `container/heap` 实现最大堆，分三阶段执行：① 无锁遍历文件系统收集所有缓存文件信息（路径、大小、mtime），② 持读锁查询 lruIndex 获取内存中的访问时间并构建最大堆（只保留最旧的 N 个文件，N = max(indexSize/4, 128)），③ 持写锁从最旧开始删除直到总量低于上限。内存开销 O(N) 而非 O(全部文件数)。超限时从最旧开始删除。
...
关键方法：`ComputeMissing`（计算从未尝试过的缺失指纹，并发度 `fpWorkerCount()` = `clamp(GOMAXPROCS/4, 1, 4)`）、`RecomputeAll`（清空指纹与「已尝试」标记后全量重算）、`Cancel`（中断在跑的任务）、`GetProgress`（返回 status/computed/total/failed）。若已有任务在运行，打断旧任务后重新启动。

**开销控制**（songloft-org/songloft#323）：扫描后自动计算默认关闭（config `scan_auto_fingerprint`）；失败也写入 `songs.fingerprint_attempted_at`，避免每轮扫描重跑同一批注定失败的文件；单文件只采样前 120 秒。

底层工具：`ExtractFingerprint` 调用 `ffmpeg [-ss] -t 120 -f chromaprint` 提取 base64 指纹（30 秒超时 + `WaitDelay`），CUE 轨按 `cue_start/end_seconds` 区间采样；`IsChromaprintAvailable` 首次调用时检测并缓存结果（走 `SetFingerprintFFmpegPath` 注入的 `ffmpeg_path` 配置）。
