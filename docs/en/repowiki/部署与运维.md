...
**Stage one: go-builder** (`ghcr.io/songloft-org/songloft-base-golang:1.26-alpine`)

- Installs the `make upx git` build toolchain (with retry)
...
**Stage two: runtime** (`ghcr.io/songloft-org/songloft-base-alpine:3.24`, pinned by digest)

- Installs `ca-certificates tzdata alsa-lib alsa-plugins alsa-utils alsa-ucm-conf` (ALSA runtime libraries)
- Copies `ffmpeg` and `ffprobe` from the `ghcr.io/songloft-org/songloft-base-ffmpeg` image to `/bin/`
...
The release workflow mirrors the upstream Docker Hub base images to GHCR first. Subsequent architecture builds read base images only from GHCR, reducing Docker Hub manifest requests and avoiding shared egress IP rate limits.
...
| `PUID` / `PGID` | empty (root) | Either one set runs the main process as that non-root uid:gid; the unset one defaults to `1000` |
| `FIX_MUSIC_PERMISSIONS` | `false` | When `true`, recursively fixes ownership of `/app/music` to `PUID:PGID` (only the top-level directory is fixed by default) |
...
**Section sources**
- [Dockerfile:119-133](https://github.com/songloft-org/songloft/blob/main/Dockerfile#L119-L133) -- VOLUME and ENV declarations
...
5. When `PUID`/`PGID` is set, chown the mounted directories then drop privileges via `su-exec` before starting the service (see the "3.4 Running as Non-Root" section below); otherwise start directly via `exec /app/data/songloft "$@"`
...
**Section sources**
- [scripts/docker-entrypoint.sh:1-292](https://github.com/songloft-org/songloft/blob/main/scripts/docker-entrypoint.sh#L1-L292) -- Complete entrypoint script
...
### 3.4 Running as Non-Root (PUID/PGID)

Setting either `PUID` or `PGID` enables non-root mode; the unset one defaults to `1000`. When neither is set, the container keeps its default root-based startup, so existing deployments are unaffected (songloft-org/songloft#380).

- `/app/data` is recursively `chown`ed to `PUID:PGID` on every startup: the directory is small (db, covers, cache, etc.), and this also fixes ownership left over from a previous root-based run — otherwise the new user couldn't open the old data
- `/app/music` only has its top-level directory `chown`ed by default, not recursively: a music library can have hundreds of thousands of files / multiple TB, and recursively walking it on every startup is not acceptable. Once the top level is writable, newly written files are created with `PUID:PGID` already correct
- `FIX_MUSIC_PERMISSIONS=true` additionally fixes `/app/music` recursively, for cleaning up files left over from before the switch to non-root — normally only needs to be run once after switching
- Privileges are finally dropped via Alpine's built-in `su-exec` (lighter than `gosu`) before executing the main program

**Section sources**
- [scripts/docker-entrypoint.sh:259-292](https://github.com/songloft-org/songloft/blob/main/scripts/docker-entrypoint.sh#L259-L292) -- PUID/PGID privilege-drop logic
- [Dockerfile:130-131](https://github.com/songloft-org/songloft/blob/main/Dockerfile#L130-L131) -- PUID/PGID environment variable declarations
...
| `PUID` / `PGID` | empty (root) | Either one set runs the main process as that non-root uid:gid |
| `FIX_MUSIC_PERMISSIONS` | `false` | When `true`, recursively fixes ownership of `/app/music` |
