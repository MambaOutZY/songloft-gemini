...
The current schema has a total of **8 tables**: `songs`, `playlists`, `playlist_songs`, `play_history`, `configs`, `auth_tokens`, `js_plugins`, and `plugin_storage`, evolved from the initial version to the current state through 30 migration files. The `updated_at` column of the core tables is automatically maintained by a SQLite TRIGGER.
