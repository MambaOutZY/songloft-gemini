...
| `FingerprintService` | optional injection | Auto-compute fingerprints after a scan completes (off by default) |
...
| `FindLyricFile` / `ReadLyricFile` | Delegates to `FindSidecarLyricFile`/`ReadSidecarLyric`; supports case variants, double-extension, UTF-16 BOM + GBK decode |
...
Key methods: `ComputeMissing` (computes missing fingerprints never attempted before, with concurrency `fpWorkerCount()` = `clamp(GOMAXPROCS/4, 1, 4)`), `RecomputeAll` (clear fingerprints plus the "attempted" markers, then fully recompute), `Cancel` (interrupt the running task), `GetProgress` (returns status/computed/total/failed). If a task is already running, it interrupts the old task before restarting.

**Cost control** (songloft-org/songloft#323): auto-compute after a scan is off by default (config `scan_auto_fingerprint`); failures are also written to `songs.fingerprint_attempted_at` so scans don't re-run the same batch of doomed files every round; each file samples only its first 120 seconds.

Underlying tools: `ExtractFingerprint` calls `ffmpeg [-ss] -t 120 -f chromaprint` to extract a base64 fingerprint (30-second timeout plus `WaitDelay`), sampling CUE tracks by their `cue_start/end_seconds` range; `IsChromaprintAvailable` detects and caches the result on first call (honoring the `ffmpeg_path` config injected via `SetFingerprintFFmpegPath`).
