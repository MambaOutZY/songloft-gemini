...
Priority: sidecar `.lrc` file > embedded lyrics (tag). `FindSidecarLyricFile` matches candidates in order:
`<base>.lrc` / `.LRC` / `.Lrc`, then `<full filename>.lrc` / `.LRC` / `.Lrc`.
Zero-byte files and same-named directories are treated as not found. `ReadSidecarLyric` handles UTF-16
BOM decoding and GBK correction (`tag.FixEncoding`). CUE split tracks skip sidecar lyrics (a whole-disc
lrc timeline applied to a single track is wrong).

At runtime, `GET /songs/{id}/lyric` also reads the sidecar from disk; on hit it takes priority over
DB-stored plugin lyrics and asynchronously syncs to DB via `SyncSidecarLyric` (without writing back to
audio tags). `manual` lyrics are never overridden by sidecar.
...
ScanAndImportAsync
  ...
  +-- go doScanAndImport(ctx, reimport)
        ...
        |
        (8) runAutoFingerprint -- automatically compute missing fingerprints (off by default, see 5.4)
```
...
```
ffmpeg [-ss <cueStart>] -i <filePath> -map 0:a:0 -map_metadata -1 -t 120 \
  -f chromaprint -fp_format base64 -
```

- **120-second sampling cap** (`fingerprintSampleSeconds`): the AcoustID de-facto standard, which also turns the cost of long audio (audiobooks, whole-disc images) into a constant. Without a cap, chromaprint decodes the entire file
- **CUE tracks sample by range**: a CUE track's `file_path` points at the whole-disc image, so `-ss cue_start_seconds` seeks into it and `-t` is clamped inside the track; otherwise every track under the same image gets an identical fingerprint
- Timeout: 30-second context timeout, plus `cmd.WaitDelay = 5s` so an ffmpeg child process holding the pipes cannot pin a worker
- Output: a base64-encoded fingerprint string (read from stdout, taking the first line)
- Duration: parsed from stderr as `Duration: HH:MM:SS.XX` (the full-file length, unaffected by `-t`); CUE tracks use the track length instead
...
| Method | Behavior |
|------|------|
| `ComputeMissing` | Compute fingerprints for all local songs that lack one and were never attempted |
| `RecomputeAll` | Clear all existing fingerprints plus the "attempted" markers, then recompute everything |
| `Cancel` | Interrupt the running task and await its teardown; progress becomes `cancelled` |

If a task is already running, the old task is canceled and awaited before the new one starts.

`doCompute`'s concurrency comes from `fpWorkerCount()` = `clamp(GOMAXPROCS/4, 1, 4)` (Go's GOMAXPROCS is cgroup-aware, so a CPU-limited Docker container converges correctly), dispatching tasks via a channel. Progress is queryable in real time via `FingerprintProgress` (status/computed/total/failed).

**Failures must be persisted**: every worker writes `songs.fingerprint_attempted_at` (unix seconds) regardless of outcome — success goes through `UpdateFingerprint`, failure through `MarkFingerprintAttempted`. `ListLocalWithoutFingerprint` filters on `fingerprint = '' AND fingerprint_attempted_at = 0`, so a failed item is attempted only once. Without persisting failures, AutoScanner re-queues the same batch of doomed files for an ffmpeg run every round (songloft-org/songloft#323). Songs interrupted by a cancel are **not** marked, so the next run resumes them.

### 5.4 Trigger Timing and Toggle

`runAutoFingerprint` at the tail of a scan (`song_service.go`) **does not run by default**: config key `scan_auto_fingerprint` defaults to `false`, business endpoint `GET/PUT /api/v1/settings/scan-auto-fingerprint`. Fingerprints only serve duplicate detection and plugin search, making them an on-demand feature; computing them library-wide keeps the CPU busy long after the scan progress already reads "completed". While off, users trigger it manually from the "Duplicate detection" page.

The fingerprint task uses its own `context.Background()` and does not reuse the scan's cancelCh (`scanProgressManager.Complete()` already did `close(cancel); cancel = nil`, after which `GetCancelChannel()` returns nil), so it is canceled separately via `POST /api/v1/scan/fingerprints/cancel`.

### 5.5 Duplicate Detection

After fingerprints are imported, `ListDuplicateGroups` can query all groups of local songs sharing the same fingerprint. Because only the first 120 seconds are sampled, songs within one fingerprint are further clustered by `fingerprint_duration` (full-file length) using a 30-second tolerance between adjacent entries (0 = unknown never splits), and only clusters of ≥2 songs count as a duplicate group — this rules out collisions such as "audiobooks/series episodes with a shared intro". The frontend displays duplicate songs via `GET /songs/duplicates` and supports deleting duplicate files via `POST /songs/batch-delete` (with `delete_files=true`).
...
| `/api/v1/scan/fingerprints/cancel` | POST | Interrupt fingerprint computation (returns `cancelled`) |
| `/api/v1/settings/scan-auto-fingerprint` | GET/PUT | "Compute fingerprints after scan" toggle, default `false` |
